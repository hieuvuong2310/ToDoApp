...
protocol Repository {
    func createOrUpdate<T>(_ value: T, _ userId: String) async -> RepositoryError? where T: Identifiable, T: Encodable, T.ID == UUID
    func read<T>(_ userId: String) async -> Result<[T], RepositoryError> where T: Decodable
}
...
extension DatabaseReference: Repository {
    ...
    func createOrUpdate<T>(_ value: T, _ userId: String) async -> RepositoryError? where T: Identifiable, T: Encodable, T.ID == UUID {
        ...
        do {
            try await  self.child(userId).child(value.id.uuidString).setValue(encodedValue)
        } catch {
            return RepositoryError.createOrUpdateError
        }
        return nil
    ...
    func read<T>(_ userId: String) async -> Result<[T], RepositoryError> where T : Decodable {
        do {
            let children = try await self.child(userId).getData().children
